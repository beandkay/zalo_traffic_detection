...
def load_all_dets(all_pred_jsons):
    ...
    return all_dets
def wbf(all_dets, iou_thr=0.9, skip_box_thr=0.001, width=1622, height=626):
    data = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_dir', type=str, default='predicts', help='directory store all json file from each model prediction')
    parser.add_argument('--output_name', type=str, default='compose_5fold.json', help='final output json')
    args = parser.parse_args()
    json_wildcard = os.path.join(args.json_dir, "*.json")
    json_files = glob.glob(json_wildcard)
    all_dets = load_all_dets(json_files)
    final_preds = wbf(all_dets, iou_thr=0.9, skip_box_thr=0.001)
