...
def zalo_image_detect(opt):
    list_model = os.listdir('/model/checkpoint')
    for model_dir in list_model:
        opt.weights = '/model/checkpoint/' + model_dir
        print(opt.weights)
        ...
        fold = model_dir.split('.')[0].split('_')[0][-1]
        num_c = model_dir.split('.')[0].split('_')[1]
        opt.json_dir = '/model/predict/json/fold_{}_{}.json'.format(fold, num_c)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_dir', type=str, default="/data/", help='directory to save results')
